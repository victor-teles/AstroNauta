...
export function vitePluginStarlightUserConfig(
	...
	const modules = {
		'virtual:astronauta/user-config': `export default ${JSON.stringify(opts)}`,
		'virtual:astronauta/project-context': `export default ${JSON.stringify({ root, srcDir })}`,
		'virtual:astronauta/user-css': opts.customCss.map((id) => `import ${resolveId(id)};`).join(''),
		'virtual:astronauta/user-images': opts.logo
			? 'src' in opts.logo
			...
			: 'export const logos = {};',
		'virtual:astronauta/components': Object.entries(opts.components)
